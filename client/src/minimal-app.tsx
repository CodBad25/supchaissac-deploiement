...
function MinimalApp() {
  ...
  const pendingCount = MOCK_SESSIONS.filter(s => s.status === 'PENDING').length;
  const validatedCount = MOCK_SESSIONS.filter(s => s.status === 'VALIDATED').length;
  const rejectedCount = MOCK_SESSIONS.filter(s => s.status === 'REJECTED').length;
  const paidCount = MOCK_SESSIONS.filter(s => s.status === 'PAID').length;
      ...
      {currentUser.role === "TEACHER" ? (
                        ...
                        {MOCK_SESSIONS.slice(0, 3).map((session) => (
                          <tr key={session.id} className="hover:bg-gray-50">
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                              {new Date(session.date).toLocaleDateString('fr-FR', { day: 'numeric', month: 'long', year: 'numeric' })} ({session.timeSlot})
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
        ...
        <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="bg-white shadow-md rounded-lg p-6">
            ...
            <p className="text-gray-600 mb-4">
              Vous êtes connecté en tant que {currentUser.role === "TEACHER" ? "enseignant" :
                currentUser.role === "SECRETARY" ? "secrétariat" :
