...
interface Session {
  ...
  teacherName: string;
  inPacte?: boolean;
  status: 'PENDING_SUBMISSION' | 'PENDING_REVIEW' | 'VALIDATED' | 'PAID';
  createdAt?: string;
  updatedAt?: string;
  updatedBy?: string;
...
import { Users, BookOpen, Clock, Sun, Moon, Camera, Upload, Download, FileSpreadsheet, FileText, CalendarIcon } from "lucide-react";
    ...
    if (sessionToConfirm.isUpdate) {
      ...
      createSessionMutation.mutate(createData, {
        onSuccess: async (newSession) => {
          ...
          // Upload du fichier Excel s'il existe
          if (sessionToConfirm?.type === 'DEVOIRS_FAITS' && devoirsFaitsForm.excelFile && newSession?.id) {
            try {
              const formData = new FormData();
              formData.append('file', devoirsFaitsForm.excelFile);

              const uploadResponse = await fetch(`/api/sessions/${newSession.id}/attachments`, {
                method: 'POST',
                body: formData,
                credentials: 'include',
              });
              ...
              if (uploadResponse.ok) {
                description += ' Fichier Excel joint avec succès.';
              } else {
                    ...
                    <Table>
                      ...
                      <TableBody>
                        {sessions.slice(0, 5).map((session) => {
                          ...
                          const canEdit = session.status === "PENDING_REVIEW" && session.teacherId === user?.id;
